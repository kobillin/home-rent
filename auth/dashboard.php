<?php
	...
	if($_SESSION['role'] == 'admin'){
		...
		$stmt = $connect->prepare('SELECT count(*) as register_land FROM registrations_land');
		$stmt->execute();
		$lands = $stmt->fetch(PDO::FETCH_ASSOC);

		$stmt = $connect->prepare('SELECT count(*) as bookings FROM bookings');
		$stmt->execute();
	...
	$stmt = $connect->prepare('SELECT count(*) as register_land FROM registrations_land WHERE user_id = :user_id');
	$stmt->execute(array(
		':user_id' => $_SESSION['id']
		));
	$lands = $stmt->fetch(PDO::FETCH_ASSOC);

	$stmt = $connect->prepare('SELECT count(*) as bookings FROM bookings WHERE bookname = :fullname');
	$stmt->execute(array(
	...
	$stmt = $connect->prepare('SELECT count(*) as apartothers FROM room_rental_registrations_apartment');
	$stmt->execute(array(
		':user_id' => $_SESSION['id']
		));
	$apartothers = $stmt->fetch(PDO::FETCH_ASSOC);

	$stmt = $connect->prepare('SELECT count(*) as landothers FROM registrations_land ');
	$stmt->execute(array(
		':user_id' => $_SESSION['id']
		));
	$landothers = $stmt->fetch(PDO::FETCH_ASSOC);

	$stmt = $connect->prepare('SELECT count(*) as roomothers FROM room_rental_registrations');
	$stmt->execute(array(
		':user_id' => $_SESSION['id']
		));
	$roomothers = $stmt->fetch(PDO::FETCH_ASSOC);
?>
					...
					<div class="row">						
						...
						<?php 
							if($_SESSION['role'] == 'admin'){ 
								...
								echo '<b>Land for Sale: <span class="badge badge-pill badge-success">'.$lands['register_land'].'</span></b>';
								echo '</div></a>';
								echo '</div>';
							} 
						?>	
						...
						<?php 
							if($_SESSION['role'] == 'admin'){ 
								echo '<div class="col-md-3">';
								echo '<a href="../app/bookings.php"><div class="alert alert-warning" role="alert">';
								echo '<b>Bookings: <span class="badge badge-pill badge-success">'.$bookings['bookings'].'</span></b>';
								echo '</div></a>';
								echo '</div>';
							} 
						?>	
						...
						<?php 
							if($_SESSION['role'] == 'admin'){ 
								...
								echo '<b>Rooms/Apartments: <span class="badge badge-pill badge-success">'.(intval($total_rent['total_rent'])+intval($total_rent_apartment['total_rent_apartment'])).'</span></b>';
								echo '</div></a>';
								echo '</div>';
							} 
						?>
